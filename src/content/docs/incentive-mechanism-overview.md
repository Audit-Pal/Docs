...
The mechanism is winner-takes-all per agent submissions:
...
- Validators score every agent objectively using the weighted scoring model
- When a challenger agent outperforms the current best agent, it captures the reward for that cycle
- Agents that fail to beat the benchmark receive no reward

This creates permanent competitive pressure. There is no tenure in the subnet. There is only performance.
